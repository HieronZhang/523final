...
void scheduling_prefetch(){
    ...
    long tolerant_line = target_mem_line * 0.7; //TODO: tune this parameter
...
int FlashNeuron_memory_manager::alloc_from_left(Tensor* tensor){
    ...
    else
    {
        ...
        std::cout<<"Allocated tensor "<<tensor->name()<<" on GPU memory: page "<<index - page_size<<" to page "<<index<<"."<<std::endl;
        return 0;
    }
}
...
int FlashNeuron_memory_manager::alloc_from_right(Tensor* tensor){
    ...
    else
    {
        ...
        std::cout<<"Allocated tensor "<<tensor->name()<<" on GPU memory: page "<<index + 1<<" to page "<<index + page_size +1<<"."<<std::endl;
        return 0;
    }
}
...
void FlashNeuron_memory_manager::dealloc_tensor(Tensor* tensor){
    ...
    std::cout<<"De-allocated tensor "<<tensor->name()<<" on GPU memory: page "<<tensor->f_page_range[0]<<" to page "<<tensor->f_page_range[1]<<"."<<std::endl;
}


FlashNeuron_simulator::FlashNeuron_simulator(double BW_ssd_GBs, double GPU_size_GB) : mem_manager(GPU_size_GB) {
    total_sim_time = 0;
...
int FlashNeuron_simulator::serve_one_pending_event(int kernel_event_id){
    ...
    if (fl_pending_event_queue.top().type==Fl_Pending_Event_Type::Kernel_Finish)
    {
        ...
        if (!fl_fetch_queue.empty() || !fl_offload_queue.empty())
        {
        ...
        if (!backward_starts) //forward
        {
            ...
            while (offload_list_index < offload_hints_fn.size())
            {
                ...
                if (offload_hints_fn[offload_list_index].issued_time==i)
                {
                    ...
                    fl_offload_queue.push(offload_event);
...
double simulate_flashneuron(){
    FlashNeuron_simulator sim(SSD_PCIe_bandwidth_GBps, GPU_memory_size_GB);
    sim.run();
    return sim.total_sim_time;
}
