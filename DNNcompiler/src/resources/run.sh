...
APPLICATION=gpg
GEN_CONFIG=configs/genconfigs.py
MAX_CONCURRENT_RUN=25
PATTERN=".*"
# parse command line args
while getopts "hgdrfkp:j:" arg; do
  case $arg in
    h)
      print_help "$@"
      exit 0
    ;;
    g)
      if ! [ -f "$base_folder/$GEN_CONFIG" ]; then
        printf "\033[0;31mConfig generation script <%s> not found\033[0m\n" "$GEN_CONFIG" >&2
        exit 4
      fi
      IFS=" " read -ra overwrite_folders <<< "$(find "$base_folder/configs" -type d -not -path "$base_folder/configs" -print0 | xargs --null)"
      ...
      python3 "$base_folder/$GEN_CONFIG"
...
for file in "$base_folder"/configs/*/*.config; do
  ...
  ret=$(sed -rn "s/is_simulation\s*([0-9]+)\s*/\1/p" "$file")
...
for file in "${sim_configs[@]}"; do
  ...
  output_folder="$base_folder"/$(sed -rn "s/output_folder\s*(\S+)\s*/\1/p" "$file")
...
if [[ $RERUN_FAILED -ne 0 ]]; then
  ...
  for file in "${rerun_configs[@]}"; do
    ...
    if ! tmux has-session -t "$group_name:$config_name" &> /dev/null; then
      ...
      tmux new-window -t "$group_name" -n "$config_name" -c "$base_folder"
