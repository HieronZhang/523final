...
#     json.dump([get_tensor_expr_info_from_op(op) for op in ops], f, indent=4)

# ele_weight_size = 0
# for op in ops:
#     if op.name not in OP_TYPE_ELEM_TWO_INPUTS:
#         continue
#     for i, should_use in enumerate(op.should_count_inputs):
#         if should_use:
#             ele_weight_size += np.prod(op.input_dict[f"input{i}"]["shape"])

# print(f"ele_weight_size: {ele_weight_size} (bytes)")
